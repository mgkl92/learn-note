...
void worker(int min, int max) {
    ...
    // remove `lock/unlock` from the for loop
    exclusive.lock();
    sum += temp;
    exclusive.unlock();
}
...
int main(int argc, char const *argv[]) {
     ...
     * Output
     * Hardware Concurrency: 4
     * Concurrent task finish, 2648487 ms consumed, Result: 2.10819e+13
     *
     * Note: The runing time is much smaller than the serial verion.
     */
    return 0;
}
